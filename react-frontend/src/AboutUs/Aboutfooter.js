import { Link } from "react-router-dom";
import React, { Component } from "react";
import $ from "jquery";


function Aboutfooter(){
    return(
        <div>
            <footer className="u-clearfix u-footer u-gradient u-footer" id="sec-9dee">
                <div className="u-clearfix u-sheet u-sheet-1">
                    <div className="u-clearfix u-expanded-width u-gutter-26 u-layout-wrap u-layout-wrap-1">
                        <div className="u-gutter-0 u-layout">
                            <div className="u-layout-row">
                                <div className="u-align-left u-container-style u-layout-cell u-left-cell u-size-60 u-layout-cell-1">
                                    <div className="u-container-layout u-valign-bottom-lg u-valign-bottom-md u-valign-bottom-sm u-valign-middle-xs u-container-layout-1">
                                        <div data-position="" className="u-position u-position-1">
                                            <div className="u-block">
                                                <div className="u-block-container u-clearfix">
                                                    <h6 className="u-block-header u-text u-text-body-alt-color">&nbsp;
                                                        <span style={{fontSize: "1.25rem"}}>CONTACT US</span><br/>
                                                        <span style={{fontSize: "0.875rem"}}>
                                                            <span className="u-file-icon u-icon u-text-white">
                                                                <img src={require("../img/About/1250663.png")} alt=""/>
                                                            </span>&nbsp;&nbsp;vnhp.auction@aptech.com
                                                        </span><br/>
                                                        <span style={{fontSize: "0.875rem"}}>
                                                            <span className="u-file-icon u-icon u-text-white">
                                                                <img src={require("../img/About/1034131.png")} alt=""/>
                                                            </span>&nbsp;&nbsp;09.9929.0999
                                                        </span><br/>
                                                        <span style={{fontSize: "0.875rem"}}>
                                                            <span className="u-file-icon u-icon u-text-white">
                                                                <img src={require("../img/About/7175143.png")} alt=""/>
                                                            </span>&nbsp;099.999.7979 
                                                        </span><br/><br/> &nbsp;
                                                        <span style={{fontSize: "1.25rem"}}>ADDRESS</span><br/>
                                                        <span style={{fontSize: "0.875rem"}}>
                                                            <span className="u-file-icon u-icon u-text-white">
                                                                <img src={require("../img/About/927667.png")} alt=""/>
                                                            </span>&nbsp;&nbsp;35/6 d5 street, 25th ward, Binh Thanh district, HCM
                                                        </span><br/><br/>
                                                        <span style={{fontSize: "0.875rem"}}>
                                                            TERM &amp; 
                                                            POLICY<br/>
                                                            PRODUCT<br/>
                                                            ABOUT US<br/>
                                                            CART
                                                        </span>
                                                    </h6>
                                                <div className="u-block-content u-text"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <img className="u-expanded-height-lg u-expanded-height-md u-expanded-height-sm u-expanded-height-xs u-hidden-sm u-hidden-xs u-image u-image-1" src={require("../img/About/image2.png")} data-image-width="1826" data-image-height="1370"/>
                                    <div className="u-hidden-xs u-social-icons u-spacing-10 u-social-icons-1">
                                        <Link className="u-social-url" title="facebook" target="_blank" to="https://facebook.com/name">
                                            <span className="u-icon u-social-facebook u-social-icon u-icon-5">
                                                <svg className="u-svg-link" preserveAspectRatio="xMidYMin slice" viewBox="0 0 112 112" >
                                                    <use></use>
                                                </svg>
                                                <svg className="u-svg-content" viewBox="0 0 112 112" x="0" y="0" id="svg-1d20">
                                                    <circle fill="currentColor" cx="56.1" cy="56.1" r="55"></circle>
                                                        <path fill="#FFFFFF" d="M73.5,31.6h-9.1c-1.4,0-3.6,0.8-3.6,3.9v8.5h12.6L72,58.3H60.8v40.8H43.9V58.3h-8V43.9h8v-9.2c0-6.7,3.1-17,17-17h12.5v13.9H73.5z"></path>
                                                </svg>
                                            </span>
                                        </Link>
                                        <Link className="u-social-url" title="twitter" target="_blank" to="https://twitter.com/name">
                                            <span className="u-icon u-social-icon u-social-twitter u-icon-6">
                                                <svg className="u-svg-link" preserveAspectRatio="xMidYMin slice" viewBox="0 0 112 112">
                                                    <use></use>
                                                </svg>
                                                <svg className="u-svg-content" viewBox="0 0 112 112" x="0" y="0" id="svg-895d">
                                                    <circle fill="currentColor" className="st0" cx="56.1" cy="56.1" r="55"></circle>
                                                    <path fill="#FFFFFF" d="M83.8,47.3c0,0.6,0,1.2,0,1.7c0,17.7-13.5,38.2-38.2,38.2C38,87.2,31,85,25,81.2c1,0.1,2.1,0.2,3.2,0.2c6.3,0,12.1-2.1,16.7-5.7c-5.9-0.1-10.8-4-12.5-9.3c0.8,0.2,1.7,0.2,2.5,0.2c1.2,0,2.4-0.2,3.5-0.5c-6.1-1.2-10.8-6.7-10.8-13.1c0-0.1,0-0.1,0-0.2c1.8,1,3.9,1.6,6.1,1.7c-3.6-2.4-6-6.5-6-11.2c0-2.5,0.7-4.8,1.8-6.7c6.6,8.1,16.5,13.5,27.6,1c-0.2-1-0.3-2-0.3-3.1c0-7.4,6-13.4,13.4-13.4c3.9,0,7.3,1.6,9.8,4.2c3.1-0.6,5.9-1.7,8.5-3.3c-1,3.1-3.1,5.8-5.9,7.4c2.7-0.3,5.3-1,7.7-2.1C88.7,43,86.4,45.4,83.8,47.3z"></path>
                                                </svg>
                                            </span>
                                        </Link>
                                        <Link className="u-social-url" title="instagram" target="_blank" to="https://instagram.com/name">
                                            <span className="u-icon u-social-icon u-social-instagram u-icon-7">
                                                <svg className="u-svg-link" preserveAspectRatio="xMidYMin slice" viewBox="0 0 112 112">
                                                    <use></use>
                                                </svg>
                                                <svg className="u-svg-content" viewBox="0 0 112 112" x="0" y="0" id="svg-0743">
                                                    <circle fill="currentColor" cx="56.1" cy="56.1" r="55"></circle>
                                                    <path fill="#FFFFFF" d="M55.9,38.2c-9.9,0-17.9,8-17.9,17.9C38,66,46,74,55.9,74c9.9,0,17.9-8,17.9-17.9C73.8,46.2,65.8,38.2,55.9,38.2z M55.9,66.4c-5.7,0-10.3-4.6-10.3-10.3c-0.1-5.7,4.6-10.3,10.3-10.3c5.7,0,10.3,4.6,10.3,10.3C66.2,61.8,61.6,66.4,55.9,66.4z"></path>
                                                    <path fill="#FFFFFF" d="M74.3,33.5c-2.3,0-4.2,1.9-4.2,4.2s1.9,4.2,4.2,4.2s4.2-1.9,4.2-4.2S76.6,33.5,74.3,33.5z"></path>
                                                    <path fill="#FFFFFF" d="M73.1,21.3H38.6c-9.7,0-17.5,7.9-17.5,17.5v34.5c0,9.7,7.9,17.6,17.5,17.6h34.5c9.7,0,17.5-7.9,17.5-17.5V38.8C90.6,29.1,82.7,21.3,73.1,21.3z M83,73.3c0,5.5-4.5,9.9-9.9,9.9H38.6c-5.5,0-9.9-4.5-9.9-9.9V38.8c0-5.5,4.5-9.9,9.9-9.9h34.5c5.5,0,9.9,4.5,9.9,9.9V73.3z"></path>
                                                </svg>
                                            </span>
                                        </Link>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
        <section class="u-backlink u-clearfix u-grey-80">
            <p class="u-text">
            <span>Copyright by</span>&nbsp;
            </p>
            <a class="u-link" href="" target="_blank">
            <span>VNHP AUCTION CORPORATION</span>
            </a>
        </section>
    </div>
    )
}

export default Aboutfooter;